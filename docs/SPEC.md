...
代わりに、進捗を消費計算量から測る。Boltz は拡散フェーズの内部で進捗を出さないが、**やるべき
計算量は機械が遅くなっても変わらない**。変わるのは待ち時間だけである。したがって、

* この規模が必要とする CPU 秒 … 完了したジョブの `cpu_seconds` から回帰（安定）
* このジョブが今得ている CPU 秒 / 実秒 … supervisor が 3 秒ごとに実測

の 2 つに分け、残り時間 = （必要 CPU 秒 − 消費 CPU 秒）÷ 実効効率 とする。スワップに落ちると
効率が落ち（実測: 1,278 トークンのジョブで実秒 30 秒あたり CPU 4.6 秒、うち 98 % がシステム時間）、
推定が自動的に伸びる。

`basis` が答えの出どころを示す。`cpu` は実測、`baseline` は履歴からの推定（推定時間内）、
`unknown` は正直に答えられない場合で、このとき `seconds` は null になる。効率が 2 % を下回る
ジョブと、推定を超過していて計算量の実測がないジョブが `unknown` に入る。
...
| バージョン | 変更 |
| --- | --- |
| 0.2.2 | 実行中ジョブの残り時間を消費計算量から実測（CPU 秒の記録と実効効率）、推定不能なキューを下限として表示 |
