...
`GET /api/jobs/eta`。待機中・実行中の全ジョブについて、履歴から回帰した所要時間を合計する。
実行中のジョブは経過時間を引いた残り（最低 15 秒）を使う。正規化できない仕様のジョブは
`unknown` に数え、合計には入れない。

```jsonc
{"jobs": 5, "counted": 5, "unknown": 0, "seconds": 1820,
 "finish_at": 1789441000.0, "now": 1789439180.0,
 "items": [{"id": "job_...", "title": "...", "status": "running", "kind": "predict", "seconds": 420}]}
```
...
`peak_memory.txt`（ピーク GB）と `live_memory.json`（3 秒ごとの現在値・スワップ量・空きディスク）を
書く。ピークは所要時間の推定と同様、履歴から回帰してトークン数から予測する。
