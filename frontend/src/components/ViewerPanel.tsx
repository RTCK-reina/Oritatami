...
export function ViewerPanel({ theme, fullscreen, onToggleFullscreen }: { theme: 'dark' | 'light'; fullscreen: boolean; onToggleFullscreen: () => void }) {
    ...
    useEffect(() => {
        ...
        import('../viewer/Viewer')
            .then(async mod => {
                ...
                offs.push(v.onError(message => toast('error', `3D ビューア: ${message}`)));
            ...
            .catch(e => !disposed && setError(`3D ビューアを起動できませんでした: ${errorMessage(e)}`));
    ...
    useEffect(() => {
        ...
        const build = async (src: ViewSource): Promise<LoadItem[]> => {
            if (src.kind === 'job') {
                ...
                if (job.kind !== 'predict' || !job.result) throw new Error('このジョブには構造がありません');
                const model = job.result.models.find(m => m.index === modelIndex) ?? job.result.models[0];
                setModelCount(job.result.models.length);
                setTitle(`${job.title}${job.result.models.length > 1 ? ` · モデル ${model.index + 1}` : ''}`);
            ...
            if (a.kind !== 'predict' || !a.result || b.kind !== 'predict' || !b.result) throw new Error('比較できる結果がありません');
            setModelCount(1);
            setTitle(`比較: ${src.fixed.title} ⇄ ${src.moving.title} · RMSD ${src.result.rmsd.toFixed(2)} Å`);
        ...
        build(view)
            ...
            .catch(e => {
                if (alive) {
                    ...
                    toast('error', `表示に失敗: ${errorMessage(e)}`);
                }
            })
    ...
    const takeShot = async (resolution: 'viewport' | 'ultra-hd', transparent = false) => {
        try {
            if (!viewer.current) throw new Error('3D ビューアの準備ができていません');
    ...
    return (
        <div className="panel viewer-panel">
            <div className="viewer-toolbar">
                <span className="viewer-title" title={title}>{title || '3D ビューア'}</span>
                {modelCount > 1 && view?.kind === 'job' && (
                    <select value={modelIndex} onChange={e => setModelIndex(Number(e.target.value))} title="サンプル (信頼度の高い順)" aria-label="モデル">
                        {Array.from({ length: modelCount }, (_, i) => <option key={i} value={i}>モデル {i + 1}</option>)}
                    </select>
                )}
                <select value={style} onChange={e => setStyle(e.target.value as RepStyle)} title="表示のしかた" aria-label="表示" disabled={!hasStructure}>
                    <option value="cartoon">リボン</option>
                    <option value="surface">表面</option>
                    <option value="putty">パテ</option>
                    <option value="ball-and-stick">原子 (棒球)</option>
                    <option value="spacefill">原子 (空間充填)</option>
                </select>
                <select value={color} onChange={e => setColor(e.target.value as ColorMode)} title="色分け" aria-label="色" disabled={!hasStructure || view?.kind === 'compare'}>
                    <option value="plddt" disabled={!plddtAvailable}>pLDDT (信頼度)</option>
                    <option value="chain">チェーン</option>
                    <option value="rainbow">N→C 虹色</option>
                    <option value="secondary">二次構造</option>
                    <option value="hydrophobicity">疎水性</option>
                </select>
                <label className="inline-toggle small" title="リガンドから 4.5 Å 以内の残基を表示">
                    <input type="checkbox" checked={pocket} onChange={e => setPocket(e.target.checked)} /> ポケット
                </label>
                <span className="toolbar-sep" />
                <button type="button" className={`icon-btn ${spin ? 'on' : ''}`} onClick={() => void toggleSpin()} title="自動回転" aria-pressed={spin} disabled={!hasStructure}><Icon name="rotate" /></button>
                <button type="button" className="icon-btn" onClick={() => viewer.current?.resetCamera()} title="視点をリセット (R)" disabled={!hasStructure}><Icon name="target" /></button>
                <Menu label={<Icon name="camera" />} title="画像" items={[
                    { label: '画像を撮る (画面の解像度)', onSelect: () => void takeShot('viewport') },
                    { label: '高解像度 (4K) で撮る', onSelect: () => void takeShot('ultra-hd') },
                    { label: '背景を透明にして撮る (4K)', onSelect: () => void takeShot('ultra-hd', true) },
                ]} />
                <button type="button" className="icon-btn" onClick={onToggleFullscreen} title={fullscreen ? '元の配置に戻す (F)' : 'ビューアを大きく表示 (F)'} aria-pressed={fullscreen}>
            ...
            <div className="viewer-host-wrap">
                ...
                {ready && !view && !error && (
                    <div className="viewer-empty">
                        ...
                        <div className="row wrap center">
                            <Button size="sm" onClick={() => void openExample(EXAMPLES.find(e => e.id === 'gfp-afdb') ?? EXAMPLES[0])}>GFP を眺めてみる</Button>
                            <Button size="sm" variant="ghost" onClick={() => showLeftTab('jobs')}>過去の結果を開く</Button>
                        </div>
                        <p className="small muted">ドラッグで回転 · スクロールで拡大 · 右ドラッグで移動 · 残基をクリックで選択</p>
                    </div>
                )}
                ...
                {view?.kind === 'compare' && (
                    <div className="legend">
                        ...
                        <span><i style={{ background: '#ff3df2' }} />変異</span>
                    </div>
                )}
                {fullscreen && <div className="fullscreen-hint small"><Kbd combo="f" /> または <Kbd combo="escape" /> で戻る</div>}
            </div>
            {shot && (
                <Modal title="スクリーンショット" onClose={() => setShot(null)} wide>
                    <img className="shot" src={shot} alt="3D ビューアの画像" />
                    <div className="row wrap">
                        <Button variant="primary" onClick={async () => {
                            try {
                                const r = await api.saveDataUrl(shotName, shot);
                                toast('success', `保存しました: ${r.path}`);
                            } catch (e) {
                                toast('error', `保存できませんでした: ${errorMessage(e)}`);
                            }
                        }}><Icon name="download" size={14} /> ダウンロードフォルダに保存</Button>
                        <Button onClick={async () => {
                            try {
                                ...
                                toast('error', `コピーできませんでした: ${errorMessage(e)}`);
                            }
                        }}>クリップボードにコピー</Button>
                    </div>
                </Modal>
